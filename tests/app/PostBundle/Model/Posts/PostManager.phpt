...
class PostManagerTest extends \Flame\Tests\MockTestCase
{
	...
	/** @var \Flame\Blog\PostBundle\Model\Posts\PostManager */
	private $postManager;
	...
	public function setUp()
	{
		...
		$this->postManager = new \Flame\Blog\PostBundle\Model\Posts\PostManager;
	}
	...
	public function testCreate()
	{
		...
		$postFacadeMock = $this->mockista->create('\Flame\Blog\PostBundle\Entity\Posts\PostFacade');
		$postFacadeMock->expects('save')
			->once();

		$this->postManager->injectPostFacade($postFacadeMock);
		$re = $this->postManager->create($data);
		Assert::true($re instanceof \Flame\Blog\PostBundle\Entity\Posts\Post);
	}
	...
	{
		$id = 1;
		$post = new \Flame\Blog\PostBundle\Entity\Posts\Post('', '');
		...
		Assert::true($result instanceof \Flame\Blog\PostBundle\Entity\Posts\Post);
	}
}
